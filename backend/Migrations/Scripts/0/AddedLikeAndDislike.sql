...
CREATE PROCEDURE MigrationsScript()
BEGIN
    IF NOT EXISTS(SELECT 1 FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20241217061519_AddedMoreDepartmentTags') THEN

    INSERT INTO `Tags` (`Id`, `Name`, `TagTypeId`)
    VALUES (41, '中央選舉委員會', 1),
    (42, '僑務委員會', 1),
    (43, '勞動部', 1),
    (44, '國軍退除役官兵輔導委員會', 1),
...
BEGIN
    ...
    INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
    VALUES ('20241217061519_AddedMoreDepartmentTags', '8.0.10');
...
CREATE PROCEDURE MigrationsScript()
BEGIN
    IF NOT EXISTS(SELECT 1 FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20250104074215_AddedLikeAndDislike') THEN

    CREATE TABLE `PostDislikedUser` (
        `DislikedPostsId` int NOT NULL,
        `DislikedUsersId` int NOT NULL,
        CONSTRAINT `PK_PostDislikedUser` PRIMARY KEY (`DislikedPostsId`, `DislikedUsersId`),
        CONSTRAINT `FK_PostDislikedUser_Posts_DislikedPostsId` FOREIGN KEY (`DislikedPostsId`) REFERENCES `Posts` (`Id`) ON DELETE CASCADE,
        CONSTRAINT `FK_PostDislikedUser_Users_DislikedUsersId` FOREIGN KEY (`DislikedUsersId`) REFERENCES `Users` (`Id`) ON DELETE CASCADE
    ) CHARACTER SET=utf8mb4;
...
BEGIN
    ...
    CREATE TABLE `PostLikedUser` (
        `LikedPostsId` int NOT NULL,
        `LikedUsersId` int NOT NULL,
        CONSTRAINT `PK_PostLikedUser` PRIMARY KEY (`LikedPostsId`, `LikedUsersId`),
        CONSTRAINT `FK_PostLikedUser_Posts_LikedPostsId` FOREIGN KEY (`LikedPostsId`) REFERENCES `Posts` (`Id`) ON DELETE CASCADE,
        CONSTRAINT `FK_PostLikedUser_Users_LikedUsersId` FOREIGN KEY (`LikedUsersId`) REFERENCES `Users` (`Id`) ON DELETE CASCADE
    ) CHARACTER SET=utf8mb4;
...
BEGIN
    ...
    CREATE INDEX `IX_PostDislikedUser_DislikedUsersId` ON `PostDislikedUser` (`DislikedUsersId`);
...
BEGIN
    ...
    CREATE INDEX `IX_PostLikedUser_LikedUsersId` ON `PostLikedUser` (`LikedUsersId`);
...
BEGIN
    ...
    INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
    VALUES ('20250104074215_AddedLikeAndDislike', '8.0.10');
